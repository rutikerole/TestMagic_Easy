...
import { FolderOpen,LogOut,UserCircle2Icon, File, CheckCircle, CheckSquare } from 'lucide-react';
...
function TesterDashboard() {
  ...
  useEffect(() => {
    ...
    if (!user || user.role !== "tester") navigate('/');
  }, [navigate]);

    
  const handleLogout = () => {
    ['userRole', 'userEmail', 'authToken', 'isLoggedIn', 'loggedInUser'].forEach(key => localStorage.removeItem(key));
  ...
  const testCasesSum = () => {
   return projects?.reduce((sum, project) => {
     return sum + (project?.totalTestCases || 0);
   }, 0);
 };

  const completedTestCasesSum = () => {
    return projects?.reduce((sum, project) => {
      return sum + (project?.completedTestCases || 0);
    }, 0);
  }

  const completedTcs = completedTestCasesSum();
  const totalTc = testCasesSum();
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 via-gray-100 to-gray-200">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              <UserCircle2Icon className="h-8 w-8 text-green-600" />
              <h1 className="text-2xl font-bold tracking-tight text-gray-800">Tester Dashboard</h1>
            </div>
            <Button
              variant="outline"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatsCard icon={<FolderOpen />}  label="Assigned Projects"   value={projects?.length} iconColor="text-blue-600" />
          <StatsCard icon={<File />}        label="Total TestCases"     value={totalTc} iconColor="text-violet-600" />
          <StatsCard icon={<CheckSquare />} label="Completed TestCases" value={completedTcs} iconColor="text-orange-600" />
          <StatsCard icon={<CheckCircle />} label="Completion Rate"     value={getCompletionPercentage(completedTcs,totalTc)} iconColor="text-green-600" />
        </div>
            ...
            <Card key={project.id} className="hover:shadow-xl transition-all duration-300 cursor-pointer rounded-2xl border border-gray-200 bg-white">
              ...
              <div className="flex gap-3 pt-4">
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => navigate(`/tester/project/${project.id}`)}
                  className="flex-1 bg-green-600 border-gray-300 text-white flex items-center gap-2 hover:bg-green-500 transition"
                >
                  Start Testing
                </Button>
              </div>
